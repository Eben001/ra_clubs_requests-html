...
async def render_page_with_wait(url, selector, script=None):
    ...
    # Wait for the specified CSS selector to be present on the page
    while not response.html.find(selector):
        await response.html.arender(script=script, reload=False)

    # Return the rendered HTML
    return response.html.html

async def parse_club_details(full_link_club, full_link_city, club_name, club_address):
    selector_to_wait_for = '.Box-sc-abq4qd-0.Lazy__Wrapper-sc-11a2pmb-0.CFObA img'
    render_script = """
        () => {
            return {
    ...
    """
    rendered_html = await render_page_with_wait(full_link_club, selector_to_wait_for, script=render_script)
    print(rendered_html)
    # try:
